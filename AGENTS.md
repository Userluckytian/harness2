...
- **主会话 = 编排者**：起草阶段计划、分发任务、全局把控、**验收**（亲自重跑证据命令）、文档与进度更新；**不直接写功能代码**。
- **实现/测试/审查 = 子代理**：按计划交接提示词独立执行；审查代理只读，按 `CODE_REVIEW.md` 出 P0/P1/P2 报告。
- 流程与角色细节：`docs/ai-framework/workflow-delegation.md`（交接必读）。  
- **新维护者/AI 入口**：`docs/HANDOFF.md`。  
- 与实施档位的关系：子代理协作是默认模式；档位只影响验收深度（低调档位须如实登记跳过项，不作全绿）。
