...
| 文件 | 内容 | 亮点 |
|------|------|------|
| **`prototype.html`** ✅ | **V2 单页交互原型（推荐看这个）**：顶部 4 个 Tab（工作台 / 代码·调用链 / 定时·桌宠 / 设置），全交互 | 浅色主题默认、rich 对话头、会话归档/删除/重命名、todo、codemap 上图下码、cron 面板 + 桌宠、QQ/飞书配置位 |
| `workspace.html` | V1 主工作区（静态） | 保留作对比 |
| `agent-trace.html` | V1 轨迹时间线（静态） | 保留作对比 |
