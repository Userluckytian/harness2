...
| **P2** | CLI 骨架：双渲染模式 + 焦点/Esc      | fullscreen/minimal 八区域；Esc 新规格（回合中永不取消、Ctrl+C 唯一取消、双击 stash/rewind、1000ms 宽限）+ 焦点环                                                                                                      | ✅（真机项待签收）            |
...
4. **真机清单（需人类/真机/key）**：终端 grok 复刻与 P2 双渲染/Esc 真机一轮；桌面 GUI（三栅拖拽/轨迹/模型配置/关窗口对话框/图片通道）；web 真实键盘 Enter 与列表刷新；README 三图；桌面真机一轮；云端厂商 key 一轮（DeepSeek/智谱/Anthropic）；QQ/飞书真机联调。
