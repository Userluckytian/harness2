...
| **内核下沉与三壳复刻 P0–P8** | 命令注册表下沉 core；CLI 单 TUI/Esc/命令面/卡片；桌面 slot+三栅+对话/轨迹/模型配置；core H-11~H-46；`@harness2/ui-shared` + `packages/web`                              | ✅ 2026-09-14 全部合入 main（`c62721d` → `c7c76a0`），P9 文档收口进行中 |
| 收尾                         | v1.0.0 发布动作（包名占用 / `NPM_TOKEN` / 推 tag）+ 真机手工签收（README 三图、桌面真机一轮、云端厂商 key）                                                             | ⬜ 待人类操作                                                           |

---

## 内核下沉与三壳复刻（P0–P9，2026-09-13/14）

> 本轮**不新增功能编号**——原 26 项功能面（下文三组）**全部保持 ✅，无一项回退**；本轮做的是「内核能力下沉 `core` + 三壳按参考项目重建（CLI 必刻 grok-build / 桌面参考 deepseek-harness / web 最小可用共用壳层）+ hermes-agent 能力对标补缺（H-\*）」。
> 起点 `c62721d`（main）→ 已合入 `c7c76a0`；计划 `docs/ai-framework/plans/2026-09-13-phase-shell-replication.md`；参考基线 grok-build `37949780` / deepseek-harness `c291e7961a` / hermes-agent `79445a4`（未拉取沿用，补拉见 OPEN.md）。

### 与原 26 项功能清单对齐

| 原编号组 | 功能面                                                              | 原状态 | 本轮变化                                                                                                                                 | 现在 |
| -------- | ------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 1–10     | P0 地基（monorepo/事件内核/轨迹/loop/工具/Provider/配置/审批/CLI）  | ✅     | 命令注册表下沉 core（P1）；CLI 单 TUI（P10 收敛，早期双渲染已删）、G-21~G-95 命令面/卡片/状态行（P2–P3）；轨迹与 undo 语义不变           | ✅   |
| 11–19    | P1 桌面与体验（桌面壳/多会话/分屏/分叉/记忆/浏览器/压缩/cron/undo） | ✅     | 桌面按 deepseek 重建 slot 四席位 + 三栅 + 侧栏（P4）；对话页视图环 + composer（P5）；轨迹页 + 模型配置页 + 设置壳（P6）                  | ✅   |
| 20–26    | P2 扩展与生态（插件/QQ/飞书/导出回放/MCP/subagent/Skills）          | ✅     | core 补齐 H-11~H-46（索引检索/分层压缩/可移植/标题/记忆自进化/工具集/并行扇出/零开销轮次/特权流/自然语言 cron）（P7）；新增 web 壳（P8） | ✅   |

### 本轮交付（按层）

| 层          | 交付要点                                                                                                                                                                      | 参考/依据          | 状态                      |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ------------------------- |
| `core`      | 命令注册表 + `describeCapabilities()`（P1）；H-11~H-46 能力与命令面 29 条（P7）                                                                                               | H-70 / hermes      | ✅（部分归存，见下）      |
| `cli`       | 单一 TUI（P10 起；早期双渲染已收敛）+ Esc 新规格（回合中永不取消、Ctrl+C 唯一取消、双击 stash/rewind、1000ms 宽限）+ 焦点环；命令面/卡片四件套/队列转向/状态行/minimal 基座   | grok-build（必刻） | ✅ 代码复刻，真机项待签收 |
| `desktop`   | slot 四席位 + 三栅（让步链按上游改正）+ 侧栏（PaneArea/SessionList 拆除）；对话页视图环 + composer；轨迹页 + 模型配置页（不手改文件闭环、密钥只写不回显）+ 设置壳/审批/工具卡 | deepseek（参考）   | ✅                        |
| `web`       | 最小壳：会话列表 + 对话页 + 流式 + 停止，走现有 serve HTTP/WS                                                                                                                 | 与桌面共用组件     | ✅（浏览器真机一轮通过）  |
| `ui-shared` | 壳层共享包（组件 + 端口层 `HarnessClient`/`Persistence`/`AttachmentReader`/`HostBridge`），零 electron/node 依赖；desktop 无复制粘贴                                          | P8 硬规则          | ✅                        |
| `gateway`   | **零改动**                                                                                                                                                                    | H-47 待拍板        | ⬜ 待人类决策             |

### 仍缺 / 归存（超出原 26 项的 hermes 对标，详见 OPEN.md）

- **flows / cron 三壳渲染（H-44、H-46、H-66~H-68）**：core 契约 + 状态机 + 用例齐备，CLI/serve/桌面**零接线** → 归存 P8/P9 后续。
- **H-42 `/replay` 入口缺**、**H-12 loop 自动压缩未切分层**（命令面已切）、**H-22 自动提炼 turn 未接**（propose/approve 与三壳注册已落）。
- **H-\* 其余 ⬜**（未纳入本轮范围）：H-15/H-16 启动看护与迁移诊断、H-23～H-25 技能生态/用户建模/人格、H-32 MCP 双向（服务端）、H-50～H-69 TUI 架构与 prompt flows 等。
- **真机/数据源类归存**：D-21 新会话作用域数据缺口、图片字节通道、桌面 `ui.follow_up_behavior` 配置通道；终端 grok 复刻与 TUI Esc 真机项。

### 架构决策落地情况（D1–D6 与 H-70）

| #        | 决策                                                             | 落地情况 | 证据 / 说明                                                                                                                                                                                                                                        |
| -------- | ---------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1       | TS / Node ≥22 + pnpm monorepo                                    | ✅       | 六包单仓（core / cli / desktop / web / gateway / ui-shared）                                                                                                                                                                                       |
| D2       | 桌面 Electron + React                                            | ✅       | `packages/desktop`；2026-09-14 起新增 `packages/web` 浏览器壳，二者共用 `@harness2/ui-shared`                                                                                                                                                      |
| D3       | 会话 = append-only JSONL 事件日志（唯一事实源）；SQLite/索引后置 | ✅       | 日志事实源不变；全文检索索引 `search.index.json`、自动标题 `title.json`、分层压缩 `compaction.layers.jsonl` 均为**可重建派生物**，`session.v1.jsonl` 字节不变且不入导出 zip（H-11/H-13/H-14；P9 任务简述中的「D2 会话日志/索引为派生物」即指本条） |
| D4       | 自研轻量插件总线                                                 | ✅       | 阶段 8（本轮未改）                                                                                                                                                                                                                                 |
| D5       | 会话内核与 UI 解耦，UI 是观察者                                  | ✅       | serve HTTP/WS（`protocolVersion=2`）；web 是继 cli/desktop/IM 之后的**第四个消费者**，未直连内核                                                                                                                                                   |
| D6       | 独立文件快照（不依赖 git）                                       | ✅       | 阶段 4（本轮未改）                                                                                                                                                                                                                                 |
| **H-70** | 斜杠命令两层分层（壳只拦必须 UI 的，其余下沉内核）               | 🟡→已落  | `packages/core/src/commands/`（`CoreCommand` + `describeCapabilities()`）+ cli 三入口统一分发；桌面/web 接线已在 P5/P6/P8 消费同一注册表                                                                                                           |

### H-47（多平台网关）——待人类决策

单进程同时跑 Telegram / Discord / Slack / WhatsApp / Signal / Email / CLI + 语音转写 + 跨平台会话连续性（hermes 对标）。**本轮未拍板：`packages/gateway` 零改动**；拍板前不得扩大网关范围（细节见 `docs/refs/refs-hermes-agent.md` H-47 与 OPEN.md）。

---

## GitHub Actions 可行性（用户关注点 5）

**结论：已实证可行**（2026-09-06 论证，2026-09-10 起在远程真实运行；P8 后为七 job——**六包 test**（core/gateway/cli/desktop/ui-shared/web 分包独立步骤）×3 平台 + 三平台 desktop build + pages 部署，站点 https://userluckytian.github.io/harness2/ ；**本地无法代验 run 号/结论（`gh` 未认证）**）。原论证依据：dsh 仓库 19 个 workflows 验证了 TS monorepo 全流程。
