...
| **内核下沉与三壳复刻 P0–P8** | 命令注册表下沉 core；CLI 双渲染/Esc/命令面/卡片；桌面 slot+三栅+对话/轨迹/模型配置；core H-11~H-46；`@harness2/ui-shared` + `packages/web`                              | ✅ 2026-09-14 全部合入 main（`c62721d` → `c7c76a0`），P9 文档收口进行中 |
...
| -------- | ------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 1–10     | P0 地基（monorepo/事件内核/轨迹/loop/工具/Provider/配置/审批/CLI）  | ✅     | 命令注册表下沉 core（P1）；CLI 双渲染模式、G-21~G-95 命令面/卡片/状态行（P2–P3）；轨迹与 undo 语义不变                                   | ✅   |
...
| `cli`       | 双渲染模式 + Esc 新规格（回合中永不取消、Ctrl+C 唯一取消、双击 stash/rewind、1000ms 宽限）+ 焦点环；命令面/卡片四件套/队列转向/状态行/minimal 基座                            | grok-build（必刻） | ✅ 代码复刻，真机项待签收 |
...
- **真机/数据源类归存**：D-21 新会话作用域数据缺口、图片字节通道、桌面 `ui.follow_up_behavior` 配置通道；终端 grok 复刻与 P2 双渲染/Esc 真机项。
