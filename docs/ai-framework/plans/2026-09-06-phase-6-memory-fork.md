...
本阶段列为「未执行 ➖（档位跳过）」（阶段 12 收口登记），本次补形式验收记录，不改变既有状态。形式验收通过 ≠ 完整验证，**不构成「已验证 ✅ 通过」声明**：本阶段未做真实模型端到端验证，记忆三态真机行为依赖真实 key（见 ③）。

**② 依据**

- 跳过项登记来源：阶段 15 计划「阶段开头：上阶段遗留」——「阶段 5 / 6 / 8 未做形式验收 · 阶段 12 收口 · 当时按档位跳过，登记为『未执行』」（⬜ B5-1）。
- 本次补登记所依据的既有验收证据（2026-09-06）：编排者验收通过（独立审查 pass-with-fixes → 2 P1 装配漏传 + 6 P2 已修复 `0ecccdc` + 覆盖缺口补测）；重跑证据 `pnpm test` **397 passed + 1 skipped**、typecheck 3 包 Done、chat mock 冒烟 exit 0；契约扩展确仅两处（`ChatRequest.system` / `memory/snapshot` 事件）、记忆红线通过。
- **未做端到端真机**：真实模型下三态记忆行为、桌面端分叉展示、记忆文件手工编辑漂移提示，均依赖真实模型 key / 真机环境（清单见 ③）。

**③ 遗留与边界（真机 / 外部依赖清单）**

1. （用户 key）真实模型下三态记忆行为：off 无注入 → ask 触发 pending → auto 直接记忆并在新会话生效（真机）
2. 分叉会话在桌面端展示与继续对话（fork 的桌面 UI 按钮属后续迭代）（真机）
3. 记忆文件手工编辑后的漂移拒写提示（MEMORY.md 用记事本改坏 § 结构）（真机）
4. 记忆内容不入 git / off 模式零写入——已由自动化红线覆盖（本次补记未重验，以既有证据为准）

**④ 附带说明**

- 阶段 15 A2-1 第 2 批已在本地统一网关上真机验证记忆三态（off/ask/auto + nudge 复盘 + 漂移拒写并生成 .bak，2026-09-09），对应上述 1/3 项的本地网关口径；真实用户环境与云端口径仍待补（A2-2 ➖）。
