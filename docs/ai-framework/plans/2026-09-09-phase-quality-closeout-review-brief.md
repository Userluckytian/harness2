...
> **被审对象：** 一名执行者按 `…-phase-quality-closeout.md` 的执行顺序产出的 12 个任务（A0–A5 / B1–B6）
...
你是本阶段的**专职代码审查者**，不写业务代码。本阶段由**一名执行者**顺序完成 12 个任务（A0–A5 / B1–B6），你在每批合入前做人工审查并把结论落盘。设专职审查者的原因：避免「实现者自评全绿」——单人执行时这条尤其重要，因为没有第二个开发者会顺带发现问题，你是唯一的外部校验。

| # | 铁律 | 含义 |
|---|------|------|
| 1 | **只读** | 不改代码、不「顺手修一下」、不提交到执行者分支、不 push、不打 tag。发现问题只写报告，由执行者自己改 |
| 2 | **不接受「应该能过」** | 任何声称通过的测试/验证，你必须自己重跑并贴真实输出 |
| 3 | **不碰真实环境** | 不读写 `~/.harness2/auth.json`（真 API key）、不改人类全局配置与审批模式、不执行 `npm publish` / `git tag` / `git push`；要复跑 A2 就用 `--home` 指向临时目录 + 人类提供的本地网关（`http://127.0.0.1:40080/v1` / `sk-unified-local` / `big-pickle`），别碰真实 home |
...
| `pnpm build` | 仅构建 |
| `pnpm lint` | B2 新增（执行顺序第 3 位）；若不存在说明 B2 未完成 |

---

## 2. 环境准备（只读工作树）

**前提：** 本仓库本地 `main` 长期领先 `origin/main`（写这份任务书时 ahead 29）且默认不 push；A0 里的备份 push **要先经人类同意**。所以远端很可能一直没有本阶段代码：

- 人类已同意 push 且 A0 完成：`git fetch --all` 后按下面建工作树。
- 否则（默认情况）：直接在开发机上建只读工作树，或让执行者临时推一个分支给你。
...
| # | 文件 | 为什么 |
|---|------|--------|
| 1 | `…-phase-quality-closeout.md` | **唯一计划文件**：执行顺序、全局约束、审查窗口、12 个任务细节 |
| 2 | 同一份文件的「## 任务」小节 | A0–A5 / B1–B6 的验收条件与提交规范（track-a / track-b 已并入并删除） |
| 3 | `git log --oneline main..<被审分支>` | 先看提交序列再逐批读 diff，比通读代码快 |
...
| 时机 | 审查对象 | 阻塞合入 | 说明 |
|------|----------|----------|------|
| Day 1 | 读文档、建只读工作树、跑出自己的基线 | — | 与执行者的 A0 并行 |
| Day 2 | A0、B1、B2 | **否（事后核）** | 重点：B1「只搬不删、零信息丢失」；B2 的 `🎨style` 提交抽查有无夹带逻辑改动 |
| Day 5 | **A1** | 是 | 全阶段最细一批 |
| Day 6 | A2 的证据 | 否 | 你不复跑真机，只核证据真实性：有无 mock 冒充、有无 key 明文、anthropic 渠道 baseUrl 是否配对 |
| Day 8 | **A3** | 是 | 安全类最严 |
| Day 10 | A4、B3 | 是 | 两批拆分一起看，重点零行为变更 |
| Day 12 | B4、A5、B5 | 是 | 文档与复审 |
| Day 12 末 | 联合验收 J-1～J-6 | 阻塞发布 | 全部合入后 |

**SLA：** 执行者声明「完成待审」后 24 小时内出结论。P0 立即直达执行者，不要只写文档。总投入约 5 个半天，散在 12 天里，不是全职 12 天。
...
- [ ] 时机正确：全量格式化排在**所有逻辑改动之前**（B2 在 A1 之前完成）；之后任何逻辑提交都不得再夹带格式化噪音
- [ ] CI 里 lint 是必过项且为绿
...
- [ ] 有无用 mock/stub 冒充真机验证？A2-1 必须真连人类提供的本地统一网关（`http://127.0.0.1:40080/v1`、模型 `big-pickle`）走真实 HTTP + SSE，**mock provider 通过 ≠ 真机通过**；A2-2（DeepSeek / 智谱 GLM / Anthropic 官方）只能用云端真 key，**本地网关通过也 ≠ 云端通过**。任一处顶替即 P0
- [ ] A2 的 anthropic 渠道 baseUrl 是否误带 `/v1`？代码会拼 `{baseUrl}/v1/messages`，带了就打不通——若执行者「测通了」却配错，说明这项没真跑
...
6. 待办与决策类结论交给执行者同步进 `OPEN.md`（B1 之后他是唯一维护人）。

---

## 12. 你不需要做的事

- ❌ A2 真机验证（人类签收；A2-1 走本地统一网关，A2-2 需云端厂商真 key）
...
- docs/ai-framework/plans/2026-09-09-phase-quality-closeout.md（唯一计划文件：执行顺序/审查窗口/全局约束/12 个任务细节）
...
不做：A2 真机验证（人类负责；A2-1 走本地统一网关 http://127.0.0.1:40080/v1 + big-pickle，A2-2 需云端厂商真 key）、发布操作、改代码、评估阶段范围。
```
...
1. 审查者缺位时回退为「执行者自评 + 只读子代理审查」，并在 `…-acceptance.md` 第 5 节注明「无独立人工审查」——这是降级，必须写明，不能默默跳过。
