...
> - 代码审查任务书（专职审查者 丙）：`docs/ai-framework/plans/2026-09-09-phase-quality-closeout-review-brief.md`
> **元规范：** `docs/ai-framework/phased-plan-driven.md`

**Goal：** 在 R2 激进-终端（T0–T5）与激进-桌面（D0–D6）大规模开工、以及 v1.0.0 正式发布之前，清掉四类欠账：**Windows 可用性 P0**、**真实模型端到端零验证**（测试环境人类已提供，见下）、**serve 发布前安全加固**、**工程规范与文档空账**。
...
**测试环境（人类已提供，A2 不再卡 key）：** 本地统一网关 `http://127.0.0.1:40080/v1` · key `sk-unified-local` · 模型 `big-pickle`（200K 上下文、**纯文本模型**）。2026-09-09 实测：OpenAI 兼容 `/v1/chat/completions` 与 Anthropic 原生 `/v1/messages` **两条协议均 200 且 SSE 正常**，因此 A2 开箱即测。配置写法、baseUrl 陷阱（openai 渠道带 `/v1`、anthropic 渠道不带）与八项清单见 track-a 的 A2；测试一律用 `--home` 临时目录，不动人类的 `~/.harness2`。
**子代理：** 启用（代码审查 + 验收）。**人工审查由专职审查者（丙）承担**，甲乙不再互审、专注实现；丙缺位时才回退为两轨互审，并在 acceptance 第 5 节注明。
**基线：** **代码基线 = `main` tip `d38fc4a`**（工作树干净）。其后只有本阶段计划文档的 `📝docs` 提交，不含任何代码改动；开工前 `git pull --rebase origin main` 取最新 main 即可。仓库路径 `D:/AI_Projects/harness2`。
...
| 轨道 | 执行者 | 阶段序列 |
|------|--------|----------|
| **A · 运行时与后端** | 甲 | A0 仓库卫生 → A1 Windows 可用性 P0 → A2 真机验证（本地网关双协议） → A3 serve 安全加固 → A4 core/server 大文件拆分 → A5 阶段 9 网关复审 |
| **B · 工程化与产品面** | 乙 | B1 OPEN.md 拆分 → B2 lint/CI 基建 → B3 cli/desktop 大文件拆分 → B4 规范文档补齐 → B5 文档与验收欠账 → B6 发布准备 |
| **C · 代码审查（不写业务代码）** | 丙 | 按下文「审查窗口」逐批审查两轨产出；结论填 acceptance 第 5 节。任务书见 `…-review-brief.md` |
...
3. **`docs/issue-log/<日期>.md` 是本机文件、不入库**（`.gitignore` 第 13 行已忽略 `docs/issue-log/*`，仓库内只跟踪 `OPEN.md` 与 `README.md`）。因此：本机日志按 `## 轨道A` / `## 轨道B` / `## 代码审查` 三个二级标题分区、只追加不重排；**但凡是别人需要看到的结论，必须落到入库文件**——证据与状态填 `…-acceptance.md`，待办与决策进 `OPEN.md`（乙统一维护）。只写在 issue-log 里等于没通知任何人。
...
**审查方：** ①**专职审查者 丙**（人工，按 `CODE_REVIEW.md` + `…-review-brief.md`）②独立只读子代理（非实现者）。两份都要，**安全类（A3）与拆分类（A4/B3）改动一份都不能省**。甲乙不互审，但有义务配合对方复跑（如 A3 完成后由乙跑 desktop smoke）。
...
| 时机 | 审查对象 | 是否阻塞合入 | 说明 |
|------|----------|--------------|------|
| Day 1 上午 | 丙读文档、装只读 worktree、跑出自己的 `pnpm test` 基线 | — | 与 S0 并行，不占用甲乙 |
| Day 1 傍晚 | A0、B1 | **不阻塞（事后核）** | S0 当天必须合入，否则两轨开不了工；重点只核 B1「只搬不删、零信息丢失」 |
| Day 3 | **A1** | 阻塞 | 全阶段最细一批。A1 不过则 S1 不成立，乙不得开始 `packages/core/**` 格式化 |
| Day 4–5 | B3 | 阻塞 | 零行为变更 |
| Day 6 | **A3**、B2 | 阻塞 | 安全类最严；B2 抽查 `🎨style` 提交无逻辑改动 |
| Day 7 | A4、A5、B4、B5 | 阻塞 | 拆分与文档 |
| Day 7 末 / Day 8 | 合流后联合验收 J-1～J-6 | 阻塞发布 | **唯一需要两轨都完成的一批** |

**SLA：** 执行者声明「完成待审」后 24 小时内出结论。发现 P0 当面/群里直达执行者，不能只写在文档里等人看。审查者预计投入约 4 个半天，散在 7 天内，不是全职 7 天。
...
| 2 | 真机清单 | **A2-1（必做）** 本地网关 openai + anthropic 两渠道八项清单逐项 pass 或已登记缺陷；**A2-2（可选）** 云端 DeepSeek / GLM / Anthropic 差异，无 key 记 ➖ | A（人类签收） |
...
| 12 | 阶段级代码审查 | 每阶段两份报告，结论 ✅/⚠️；❌ 下放 | 丙 + 子代理 |
...
| 本地网关通过 ≠ 云端厂商通过 | A2-1 本地双协议必做（环境已就绪）；A2-2 云端缺 key 时记 ➖，**不得用 A2-1 或 stub 顶替**；网关本身不可用则 A2 顺延、不阻塞 A3 |
...
| 双轨同日写 issue-log 冲突 | 二级标题分区、只追加不重排（该文件不入库，仅本机留痕） |
| 审查者成为瓶颈、卡住合流 | 24h 出结论 SLA；A0/B1 事后核不阻塞；丙缺位立即回退为两轨互审并在 acceptance 第 5 节注明 |
...
基线：代码基线 main tip d38fc4a（其后只有计划文档提交），工作树干净。从最新 main 建自己的独立 worktree，别在主工作树切分支，
...
每个阶段收尾：跑 pnpm -r typecheck 与 pnpm test → 请专职审查者（丙）人工审查 + 只读子代理审查 →
