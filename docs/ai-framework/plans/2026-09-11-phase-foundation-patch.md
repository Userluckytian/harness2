...
> **角色：** **单人串行前置**阶段。本阶段合入 main 并宣布冻结之前，终端 T 轨（T0–T5）与桌面 D 轨（D0–D6）**都不得开工**。
...
P 阶段期间，另一人**不空等也不动手**：读计划与研究文档、设计 fixture、草拟失败用例，**但不合入任何代码**。
...
| P 阶段拖长，挡住两人                    | P 限时；另一人同期做**只读预研**（fixture、失败用例草案），不合入               |
