...
| #   | 标准           | 通过条件                                                                                                                                      | 验证责任人                  | V0 实测（2026-09-17，命令输出见 `docs/issue-log/2026-09-17.md`）                                                                                                                                                                                                                                                                                                                                                               |
| --- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 0   | 基线记录       | Task 1 的基线 commit 短 hash 已回填本文件                                                                                                     | 执行方                      | ✅ 基线 commit `7bb5597`（V0 首个提交，分支基点 `aecd176`＝`feat/phase-p11-tui-polish` HEAD）                                                                                                                                                                                                                                                                                                                                  |
| 1   | 构建 + 类型    | `pnpm -r typecheck` exit 0                                                                                                                    | 执行方                      | ✅ 复跑两次均 exit 0，6 包 typecheck 全 Done                                                                                                                                                                                                                                                                                                                                                                                   |
| 2   | 测试基线未退化 | `pnpm test` 不低于 1120 passed + 2 skipped                                                                                                    | 执行方                      | ⚠️ 数值远超：core 1232+2 · gateway 40 · ui-shared 70 · web 36 · cli 1575+2 · desktop 930+1 = **全仓 3883 passed + 5 skipped**；但 `pnpm test` **exit 1**——desktop 2 个预存收集期红文件（V0 前既有，已用 `git diff main..HEAD -- packages/desktop` 为空取证），详见问题日志 V0-3 与 OPEN.md 第一节                                                                                                                              |
| 2b  | 代码审查       | **不适用**（无代码改动），文档自查表已填                                                                                                      | 执行方 + 编排者复核         | ✅ 不适用（无代码改动），文档自查见 `docs/issue-log/2026-09-17.md`                                                                                                                                                                                                                                                                                                                                                             |
| 3   | diary 补齐     | `docs/diary/` 存在 2026-09-10 ~ 09-17 共 8 篇，无编造                                                                                         | **编排者亲自抽查 3 篇**     | ✅ 8 篇齐全（09-10~09-17）；机器抽查：7 篇补记日记共引用的全部短哈希 **0 处**「提交不存在」或「日期与所属日记不符」；反推不出的细节一律写「无记录」                                                                                                                                                                                                                                                                            |
| 4   | OPEN.md 归整   | 两大块结构成立；开放项条数不减少                                                                                                              | 编排者                      | ⚠️ 成立但为**三大块**（计划只要求两大块，V0 另加「三、已关闭（本轮）」）；条数 40 → 46，**0 条删除**（脚本逐字校验：33 条逐字命中 + 7 条仅追加「归属阶段」列）                                                                                                                                                                                                                                                                 |
| 5   | grok 缺帧补齐  | `out/B2-long-code-block/grok/`、`out/D2-tool-expand/grok/` 各有完整三件套且 `settled:true`；**或**失败已如实登记并在记分卡标「缺基准帧」      | **编排者亲自核对 log.json** | ⚠️ 走「**或**」分支：两次尝试用满均失败（第 1 次 Enter 被启动弹窗吞掉、消息未发送；第 2 次消息已发出但 grok 上游返回 `403 FreeTierError`），`settled` 三次全为 `true` 却无有效内容——已隔离为 `out/<场景>/grok__INVALID-grok403/`、记分卡标「缺基准帧，无法评分」、并登记 OPEN.md 待人类裁决                                                                                                                                    |
| 6   | 记分卡骨架     | `docs/tui-parity/PARITY-SCORECARD.md` 存在，15 场景行齐全，估值列已填且标注为估值                                                             | 编排者                      | ⚠️ 文件存在；**18 行**（计划写 15 行且表头 `/15`，与实际 `scenarios/` 的 18 个 JSON 不符，按计划 Task 5 自查第 3 条「以实际目录为准」修正）；字符吻合度全部以 `~NN%` 形式标注为估值，配色吻合度**留空并写明原因**（`.txt` 不含颜色，无法按计划 Task 5 第 2 步「看 .txt 估配色」）                                                                                                                                              |
| 7   | G-8x 冻结标注  | 25 条 🟡 均有冻结备注，状态图标未被篡改                                                                                                       | 编排者                      | ✅ 实测该区间 🟡 恰为 25 条（与计划所述一致），25 条全部加注「本轮冻结」，重复加注 0；图标未改：区间内 🟡 仍 25 次、➖ 仍 2 条、✅ 行 16 条未动                                                                                                                                                                                                                                                                                |
| 8   | 红线核查       | `git diff --stat main -- packages/core packages/gateway` 为空；`api-surface-baseline.json` 0 行 diff；`git diff --stat main -- packages` 为空 | 编排者                      | ⚠️ 对 **V0 基点 `aecd176`** 全部为空（`packages/**` 零改动、`api-surface-baseline.json` 零改动）✅；但**按字面对 `main`** 则非空：`packages/core/src/commands/types.ts`(4 +-)、`capabilities.ts`(2 +-)、以及 `git diff --stat main -- packages` = 109 files（均系 P10/P11 未合入 main 的既有改动，**非 V0 引入**）。原因：分支基点经领导批准由 `main` 改为 `aecd176`（`main` 上没有 `scripts/tui-parity/`，Task 4/5 无法执行） |
| 9   | 密钥           | `git ls-files` 无敏感文件；`out/`、`.venv/` 未入库                                                                                            | 编排者                      | ✅ 跟踪清单里没有任何以 env、secret、credential、pem、key、id\_rsa、api-key 命名的文件（计数为 0）；`scripts/tui-parity/` 下的 `out` 与 `.venv` 两个目录也都未被跟踪（计数为 0）。另需留意：grok 自身的配置文件（用户目录下，不在本仓）里含有真实 API key，本仓不碰它、也不得把它的内容抄进任何文档                                                                                                                            |
