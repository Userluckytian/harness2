...
| 日期 | 事项 | 状态 | 详情 |
|------|------|------|------|
| 2026-09-11 | **D 轨 P1-1（阻塞合入）**：`SidePanel.tsx:26` 读 `state.selectedId`，而主路径 `controller.assignToPane` 不调 `store.select` → 六页签（对话/工具/审批/事件/文件/设置）数据源永不写入，分屏右侧恒为空。修法：`assignToPane` 内同步 `select` + `refreshAllViews` + `refreshAuthoritativeState`，并补一条接线用例锁住 | 修复中（乙） | 本地 `2026-09-11-review-D.md` §7 |
| 2026-09-11 | **两轨 P2 下放清单（非阻塞）**：D=①cron 帧造 `undefined` 幽灵流 ②force 无去重且 `resumeSession` 渲染层零调用 ③草稿写盘非原子 ④模式切换按钮不落地（逼近「不摆可点击假入口」红线）；T=①`useTurnStream` 50ms timer 无 unmount 清理 ②E2E 兜底 key 字面量 ③无 `SIGTERM`/`SIGHUP` 恢复。另：redo 渲染层零入口、F7 故障注入、文件树 IPC、主进程关窗口对话框自动化测试 | 待处理 | 本地 `…-review-T.md` / `…-review-D.md` |
| 2026-09-11 | **CI windows 抖动（新登记）**：`packages/core/test/browser.test.ts:255` 的 `expect(peak).toBe(2)` 在 CI windows 高负载下取到 1（run #61 attempt 1 `AssertionError: expected 1 to be 2`；attempt 2 重跑全绿，本机 Windows 全量亦绿）。属测试侧计时假设（三任务各 sleep 120ms，靠重叠取峰值），非生产缺陷。附带风险：`pnpm -r test` 在 core 失败即中断，gateway/cli/desktop 结果被掩盖。修法建议：用屏障让两任务确定性重叠，或断言改 `≤2` + 另证排队。文件在冻结区，需下一个 `fix/*` 窗口 | 待处理 | run #61 |
