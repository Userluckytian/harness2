...
本文的复刻矩阵即上列各条目分组小节的条目表本身，实际列名为：ID / 条目 / 要点 / 等级 / 状态（依据路径统一见「权威依据」小节，落点见「与 harness2 当前实现的差距」）。各分组索引如下（条目数与状态分布按 2026-09-14 P9 归存后正文实际统计；合计 63 条）：

| 分组小节                                                   | ID 区间    | 条目数 | 状态分布                    |
| ---------------------------------------------------------- | ---------- | ------ | --------------------------- |
| D-0x 组装模型（slot 系统）                                 | D-01～D-04 | 4      | 3 ✅ · 1 🟡                 |
| D-1x 三栅布局（`ui-layout`）                               | D-10～D-17 | 8      | 7 ✅ · 1 ➖                 |
| D-2x 侧栏与会话列表（`ui-sidebar` + `ui-workspace`）       | D-20～D-25 | 6      | 5 ✅ · 1 🟡                 |
| D-3x 对话页面（`ui-conversation` + `ui-chat`）             | D-30～D-39 | 10     | 6 ✅ · 4 🟡                 |
| D-4x 轨迹视图（`ui-trajectory`）                           | D-40～D-47 | 8      | 8 ✅                        |
| D-5x 模型配置页（`ui-settings-models`）                    | D-50～D-59 | 10     | 9 ✅ · 1 🟡                 |
| D-6x 其余可借鉴模块（包名即壳层能力清单）                  | —          | —      | 非条目表（包清单段落）      |
| D-7x 工程与文档规范（可直接借用）                          | D-70～D-72 | 3      | 3 ⬜                        |
| D-73+ 右栏停靠与新表面（2026-09-13 增补，基线 c291e7961a） | D-73～D-86 | 14     | 12 ⬜ · 2 ✅                |
| 合计（条目表分组）                                         | —          | 63     | 40 ✅ · 7 🟡 · 15 ⬜ · 1 ➖ |
...
| D-03 | ctx 服务面  | `ctx.clientModules` `ctx.modules` `ctx.connection` `ctx.fileUpload` `ctx.locale` `ctx.uiRenderer` `ctx.layout` `ctx.uiConversation` `ctx.uiSession` `ctx.slots` `ctx.conversation`；c291e7961a 增 `ctx.resources`（资源模型）、`ctx.sidebarRight` / `ctx.sidebarRightTabs`（右侧 Sidebar 导航与 tab 类型注册表）【2026-09-13 修正：服务面扩充】 | 参考 | 🟡（归存：等价物已落——slot/ctx 由 `renderer/slots/` + preload IPC 承接；`resources` / `sidebarRight` 等新增服务面随 D-73+ 未排期） |
...
| D-21 | 新会话按钮   | 工作区选择优先级：显式指定 → 当前会话所属 → 最近活跃 → 空白                                                      | 参考 | 🟡（归存：P9 后另行排期——四级降级逻辑与用例已就位；本仓无 Workspace 概念（最接近 `SessionMeta.cwd`）、`newSession()` 无作用域入参，装配层恒落空白档，待工作区数据源） |
...
| ID   | 条目             | 要点                                                                                                                                                                                                                                                                            | 等级 | 状态                                                                                                                                                                 |
| ---- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-30 | 与 target 无关   | 对话装配层不绑定具体渲染目标；`UiConversation.events` / `.views` 是唯一注册表（拒重复 key、保注册顺序、幂等 disposer）                                                                                                                                                          | 参考 | 🟡（归存：P9 后另行排期——`views` 注册表已落；`events` 注册表未建模）                                                                                                 |
...
| D-35 | 繁忙态 Enter     | 设置二选：**Queue**（进 QueueDock）或 **Steer**（pending-steering）；空闲态进 transcript                                                                                                                                                                                        | 参考 | 🟡（归存：P9 后另行排期——queue/steer 意图投递已落且与 CLI 共用 core 同一套 API；桌面无 `[ui].follow_up_behavior` 配置通道（硬编码 queue），空草稿加速 steer 未实现） |
...
| D-37 | 附件             | 图片走 FileReader data URL；文件走 FIFO 上传队列，`maxConcurrentFileUploads` 默认 2                                                                                                                                                                                             | 参考 | 🟡（归存：P9 后另行排期——data URL + FIFO 并发 2 已落；图片字节通道缺（如实标 `IMAGE_TRANSPORT_MISSING`，未伪造），文件上传通道同缺）                                 |
...
| D-39 | 图片 URL 缓存    | `ctx.uiConversation.imageUrl(sessionId, attachment)` 逐会话缓存，Chat 与 Trajectory 共用一次授权读取                                                                                                                                                                            | 参考 | 🟡（归存：P9 后另行排期——逐会话缓存已落；桌面授权读取通道缺，`imageUrl` 恒 null 降级）                                                                               |
...
| D-55 | 发现模型     | “获取可用模型”拿表单当前端点去查；回复打开**可搜索选择器而非直写**，点“添加所选”才写入；已有行保留用户调过的值                                                              | 参考 | 🟡（归存：维持现状不收口——上游用表单草稿密钥探测可用模型，本实现只读 auth.json/env，新建提供方须先保存密钥；放宽需让明文再走一遍 IPC，收益小于扩大凭据面，已在代码注释与 OPEN.md 登记） |
...
| ID   | 条目         | 要点                                                                                                            | 等级 | 状态                                                                                                                                                |
| ---- | ------------ | --------------------------------------------------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-70 | 每包 README  | 固定小节：概述 / 使用本包 / 理解实现 / 进一步探索 / 模型体验（含 KV Cache 影响）/ 已知限制与延期工作 / 开发备注 | 参考 | ⬜（归存：后续阶段未排期——本仓文档规范在 `AGENTS.md`/`docs/`，各包 README 未采用该 7 小节模板（无「模型体验 / KV Cache」节））                      |
| D-71 | 运行时不变式 | 每包 README 末尾写一行“运行时不变式”（是否发伴生入口、是否持有跳插件可变关系）                                  | 参考 | ⬜（归存：后续阶段未排期——导出面不变式由 `docs/API-STABILITY.md` + `api-surface-baseline.json` 单点承接，不逐包 README 声明）                       |
| D-72 | 已知限制小节 | 把“故意不做的事”写进文档（与本仓 `➖` 理由同构）                                                                | 参考 | ⬜（归存：后续阶段未排期——本仓已有等价实践（本文档「不采纳清单」、`docs/issue-log/OPEN.md` 已知限制、`DECISIONS.md`），但未固定为包级 README 小节） |

## D-73+ 右栏停靠与新表面（2026-09-13 增补，基线 c291e7961a）

| ID   | 条目          | 要点                                                                                                                                                                                                                                                                                                                                                                                                                     | 等级 | 状态                                                                                                                                 |
| ---- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------ |
| D-73 | 停靠布局引擎  | `ui-dockkit`：纯逻辑分裂树引擎（无框架无 DOM）+ 可逆操作（`applyOp` 同时返回逆操作）+ 线性历史 `Sequencer`（一次手势一条记录）；浮窗即 `host='float'` 的格；交互规则：手势捕获指针、焦点落 click 而非按下、chip 让位而 tab 条末端控件永不让位、chip 下限 80px 且标题不加省略号                                                                                                                                           | 参考 | ⬜（归存：后续阶段未排期——本仓为固定三栅（D-10～D-17 ✅），无分裂树停靠引擎；D-74～D-77 均依赖它）                                   |
| D-74 | 右侧 Sidebar  | `ui-sidebar-right`：每会话一个停靠面，`push`（贴右栏）/ `fullscreen`（覆盖窗口并保留宽屏轨道；窗口 <768px 打开右栏自动全屏）两形态；`ctx.sidebarRight.openResource` / `openTab` 导航控制器 + `ctx.sidebarRightTabs` 两阶段类型注册表（`extension` > `builtin` > `fallback` 档位）；引导页默认页规则：唯一停靠 tab 的引导页不可关闭，关其它唯一 tab 连带收起整列；最多左右两格、分隔条限 20%～80%；状态仅内存、刷新即折叠 | 参考 | ⬜（归存：后续阶段未排期——`rightbar` 为固定侧栏（D-10 ✅），无每会话停靠面 / fullscreen 两形态 / `sidebarRightTabs` 注册表）         |
| D-75 | 文件树 tab    | `ui-sidebar-files`：kind `files`、档位 `builtin`、无 patterns（经引导页进入）；以会话 cwd 为根逐层经网络列出，目录优先、自然序不分大小写；文件行经 `dsh-resource://file/...` 打开到本 pane；截断 / 空层 / 失败层如实显示；无 cwd 的会话只显示说明行                                                                                                                                                                      | 参考 | ⬜（归存：后续阶段未排期——无右栏 tab 体系与 `dsh-resource://` 资源协议；文件树 IPC 为已知技术尾巴，未排期）                          |
| D-76 | 文档预览 tab  | `ui-sidebar-documentpreview`：kind `text`、pattern `dsh-resource://file/**`、档位 `fallback`；文本类分页累计渲染（Markdown / 代码 / 纯文本），PDF / HTML / 图片取整字节；HTML 走 `sandbox="allow-scripts"` Blob iframe（无 allow-same-origin，关联资源 4MiB/32MiB/64 个上限）；SVG 不进 DOM/iframe；`openResource` 带 `{ line }` 1 起算行号导航；渲染器经 `ctx.documentPreviews` 注册、正文可下拉切换                    | 参考 | ⬜（归存：后续阶段未排期——无资源协议与预览器注册面；HTML `sandbox` iframe 预览未落地）                                               |
| D-77 | 资源模型      | `resources`：`dsh-resource://<type>/…` 协议提供方注册（每协议恰一个，重复注册抛错）；任意 slot 组件经 `useResource(address)` 读 `{ status, value, failure }`（none/loading/live/failed）；`ctx.resources.pin(address, signal)` 不订阅也钉住流；失败是帧不是抛错                                                                                                                                                          | 参考 | ⬜（归存：后续阶段未排期——无 `useResource` / 协议注册面；数据经 preload IPC 通道（`ui-shared` 端口层）取）                           |
| D-78 | Open In...    | `ui-open-in-app`：会话头部 `conversation.session.header.utilities` 分体按钮；主按钮在记住的应用中打开会话 workspace 目录，下拉列出宿主探测到的 catalog 应用（真实应用图标）；选择持久化 `dsh.open-in-app.choice`；飞行超 250ms 才显等待态、失败显错误 tooltip 红描边 2 秒；无 cwd 或无可用应用则整个按钮不渲染                                                                                                           | 参考 | ⬜（归存：后续阶段未排期——无宿主应用探测 / catalog 面（Electron `shell.openPath` 未接））                                            |
| D-79 | 全局面板入口  | 左侧栏 root 作用域 `sidebar.panellist` list 席位列图标入口（`id` + 可选 `order` + `label`）；`ctx.layout.selectPanel(id)` / `null` 在 `main` keyed 面板与会话界面间切换（不动当前会话），`beginNavigation()` 发导航 abort signal；`ui-workspace.openSession(id)` 选会话并令主区回会话界面；全局面板活跃时会话行不显选中样式                                                                                              | 参考 | ⬜（归存：后续阶段未排期——`main` 席位按视图环切视图（D-32 ✅），无 `sidebar.panellist` 全局 list 席位与 `selectPanel` 导航）         |
| D-80 | 聊天引用预览  | `ui-chat`：已发送文件引用与 skill 引用悬停/聚焦显虚线下划线，点击在右栏开预览（会话/目录/命令标签仍只作展示）；已完成轮次操作页脚位于正文或扩展内容下方 20px；流式贴底滚动归属即时更新、读者移动经采样周期或 `scrollend` 才生效                                                                                                                                                                                          | 参考 | ⬜（归存：后续阶段未排期——P5 已移除旧 `@path` 内建解析（引用 chip 保留但无生产调用者），无右栏预览落点（依赖 D-74））                |
| D-81 | 显式交付卡片  | `ui-deliverables`：`present` 工具声明交付最终文件（含 Bash 产物）；单交付为横满内容区卡片、多交付为 10px 间距双列网格（卡高 60px、20px 图标、>4 个默认收起）；点卡片在右栏预览，右箭头菜单提供宿主默认应用 / Finder / 资源管理器；行内代码引用同样路由右栏预览                                                                                                                                                           | 参考 | ⬜（归存：后续阶段未排期——无 `present` 工具与交付卡片；工具卡见 `renderer/tool/**`）                                                 |
| D-82 | 反馈弹窗      | `ui-message-feedback`：赞 / 踩均打开弹窗（7 个分类标签 + 详情框，皆可空），提交才记录评分并弹感谢 toast；再点已记录评分直接撤回不开弹窗；不带文本的 `/feedback`（菜单选中或输入发送）开同一弹窗，带文本仍走宿主命令；失败保留弹窗 + 警告 toast                                                                                                                                                                           | 参考 | ⬜（归存：后续阶段未排期——无反馈弹窗与对应 IPC 通道）                                                                                |
| D-83 | 命令面扩展    | `ui-commands`：新增 `action` 派发（消费触发 token 运行回调，不提交消息）；composer `+` 按钮与键入 `/` 开同一菜单，「添加」「指令」分节、每行图标 + 标题 + 说明；内置行按稳定 `definitionId` 取本地化标题/图标，中英文写法经同一会话目录解析                                                                                                                                                                              | 参考 | ⬜（归存：后续阶段未排期——composer 斜杠为行首样式文本（D-33 ✅），无 `+` 按钮菜单与 `definitionId` 本地化面；命令注册表归属见 H-70） |
| D-84 | preset 可见性 | `ui-agent-preset`：设置分区 `modeSelectionEnabled` 开关控制新建会话 chip 显隐；关闭期间宿主按部署默认（随附 Web bundle 为 `standard`）组装未指名会话，重开恢复已存用户默认；运行中 / 历史会话的组装与标签不变                                                                                                                                                                                                            | 参考 | ⬜（归存：后续阶段未排期——本仓无 agent preset 概念与 `modeSelectionEnabled` 设置分区（`SettingsConfigShape` 无该节））               |
...
D-01/D-02/D-04（slot 注册表、功能包填席、源平面拆分）、D-10～D-16（AppFrame 四席位、拖拽缩放、让步链、56px 轨道、几何瞬时、主题呈现器、降动效）、D-20/D-22～D-25（品牌行、收起动画、区域席位、滚动条可供性、版本徐标）、D-31～D-34/D-36/D-38（视图选择规则、视图环、常驻 composer、乐观提交同事务、主按钮矩阵、composer 链）、D-40～D-47（轨迹页八条：入口/记录表/时间概览/概览交互/检查器/虚拟化/壳义务/诚实性）、D-50～D-54/D-56～D-59（模型配置页除 D-55 外全部，含密钥只写不回显、校验、revision 冲突、删除与首运行）、D-85 端点校验、D-86 工具卡单视图。

### ② 部分完成待续（🟡 7 条）

| ID   | 已落部分                                           | 剩余部分与归口                                                     |
| ---- | -------------------------------------------------- | ------------------------------------------------------------------ |
| D-03 | slot/ctx 等价物（`renderer/slots/` + preload IPC） | `resources`/`sidebarRight` 等新增服务面未排期                      |
| D-21 | 新会话作用域四级降级逻辑与用例                     | 无 Workspace 数据源（`newSession()` 无作用域入参），P9 后另行排期  |
| D-30 | `views` 注册表                                     | `events` 注册表未建模，P9 后另行排期                               |
| D-35 | queue/steer 意图投递（与 CLI 共用 core API）       | 桌面无 `[ui].follow_up_behavior` 配置通道；空草稿加速 steer 未实现 |
| D-37 | data URL + FIFO 上传队列（并发 2）                 | 图片/文件字节通道缺（如实降级，不伪造），归核心加性形状            |
| D-39 | 逐会话图片 URL 缓存                                | 桌面授权读取通道缺（`imageUrl` 恒 null 降级）                      |
| D-55 | 发现模型四条字面行为                               | **维持现状不收口**：放宽需扩大凭据面，收益不成比例（已注释登记）   |

### ③ 未排期（⬜ 15 条）

D-70 每包 README 模板、D-71 运行时不变式行、D-72 已知限制小节（本仓已有等价实践但未固定为包级小节）、D-73 停靠布局引擎（dockkit）、D-74 右侧 Sidebar、D-75 文件树 tab、D-76 文档预览 tab、D-77 资源模型（`dsh-resource://`）、D-78 Open In...、D-79 全局面板入口、D-80 聊天引用预览、D-81 显式交付卡片、D-82 反馈弹窗、D-83 命令面扩展（`action` 派发）、D-84 agent preset 可见性。

**共性事实**：D-73～D-84 共用一条依赖链——本仓为固定三栅（D-10～D-17 ✅）且无资源协议与每会话停靠面，故整批随「P9 后另行排期」；启动前须按 `docs/refs/README.md` §2 重新拉取上游判定过期。

### ④ 不采纳（➖ 1 条）

D-17 挤压重排期间的滚动锚定：上游自认缺陷，不复刻缺陷（已在「不采纳清单」登记）。
...
> **口径提示（2026-09-14 P9）**：下表为 2026-09-13 首版快照（P4 之前），**未随 P4～P8 实施结果刷新**（例如 PaneArea/SessionList 已删除、三栅与 slot 已落、对话页视图环与乐观提交已落、模型配置页已可用）。本表刷新归存「P9 后另行排期」，当前判断以各条目表状态列为准，不以本表为准。
...
| 2026-09-14 | 编排会话（P9 收口 A）         | `c291e7961a` | **硬规则 #2 收口：15 条 ⬜（D-70～D-84）逐条补归存 + 7 条 🟡 悬空归口重述（其中 D-55 定为「维持现状不收口」）**；新增「归存汇总（2026-09-14 P9）」小节（✅40 · 🟡7 · ⬜15 · ➖1，合计 63）；索引摘要表按正文实际统计重算（旧表记 D-01～D-59 全 ⬜、D-73+ ⬜14，实际为 ✅40/🟡7/⬜15/➖1）；补「差距表为 P4 前快照」口径提示。**状态符号无变更**（15 条 ⬜ 全部维持 ⬜ + 归存说明，不虚标 ✅） |
