...
| D-13 | 侧栏收起     | 收起后保留 **56px 控制轨道**（不是彻底消失）；窗口宽度低于 1024px 自动收起；**仅在该窄屏态下**打开右栏才会收起「手动展开」的侧栏（宽屏打开右栏不动侧栏，依据上游 `stores.ts:131`）【2026-09-14 修正：原文缺窄屏限定，曾被实现成宽屏也无条件收起（P4 审查 P0-2）】【2026-09-13 增补】；极窄窗口下 56px 轨道仍保留（上游已知限制明示）                    | 参考   | ✅   |
| D-14 | 面板几何瞬时 | 宽度/开关状态**不持久化**：上游口径 = **reload（重挂载）即重置**；宽度是**框架级共享偏好、非每会话属性**（挂在 root store 的 `layoutInfo` 上，切会话不重置、也不按会话分别记忆）。【2026-09-14 修正】旧文案「刷新或**切会话**即重置」把宽度误当每会话属性，按上游 `packages/client/ui-layout/src/client/stores.ts` 改正                                 | 参考   | ✅   |
