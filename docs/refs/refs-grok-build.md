...
| G-8x 斜杠命令全集（依据 `04-slash-commands.md`）      | G-50～G-90 | 41     | 14 ✅ · 25 🟡 · 2 ➖ · **2026-09-17 本轮冻结**（V0–V6 不排期） |
| G-9x 基线刷新新增条目（37949780，2026-09-13）         | G-91～G-95 | 5      | 5 ⬜                                                           |
| 合计                                                  | G-01～G-95 | 95     | 44 ✅ · 43 🟡 · 5 ⬜ · 3 ➖                                    |

> **G-8x 本轮冻结（2026-09-17 阶段 V0 加注）**：G-50～G-90 区间内的 25 条 🟡 已在状态列文字后追加「本轮冻结」备注，**状态图标一个都没改**（它们确实仍是 🟡，改成 ➖ 会失真）。依据 `docs/ai-framework/plans/2026-09-17-visual-parity-program.md` §1.2 非目标——领导验收只看画面，这 25 条命令面缺口属手感/功能复刻，本总纲期间不投人力，收口后重评。
...
| G-52 | 技能升为命令 | skill 可声明 `user-invocable`，自动出现在菜单                         | 🟡（归存：P9 后另行排期——`/skills` 只读列出已落；skill 声明 `user-invocable` 自动进菜单未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                    |
| G-53 | 名字冲突     | 冲突时用 `/plugin-name:login` 形式限定，并在菜单打 badge 区分来源     | 🟡（归存：P9 后另行排期——来源 badge 已落（`tui/commands/palette-model.ts`）；`/plugin-name:login` 限定形式未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评） |
...
| G-56 | `/dashboard`（`/agents-dashboard` `/sessions`） | agents 仪表盘，`Ctrl+\` 开，仅 fullscreen                                    | 参考 | 🟡（归存：P9 后另行排期——本仓无 agents dashboard（与 G-41/G-94 同因）） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                            |
...
| G-62 | `/copy [n 或 path]`                             | 复制回复；备份到 `~/.grok/last-copy.txt`，`GROK_COPY_FILE` 可改              | 必刻 | 🟡（归存：P9 后另行排期——`Ctrl+C` 复制（OSC52）已落；`/copy [n 或 path]` 命令与 `~/.grok/last-copy.txt` 备份未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评） |
| G-63 | `/export`                                       | 导出会话                                                                     | 必刻 | ✅                                                                                                                                                                                    |
| G-64 | `/delete`                                       | 删会话；选择器内 `d` 后 `y` 确认，仪表盘 `Ctrl+X` 两次                       | 必刻 | 🟡（归存：P9 后另行排期——无删会话命令与选择器内 `d`→`y` 确认） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                                     |
| G-65 | `/rename`（`/title`）                           | 重命名，支持 `--auto`                                                        | 必刻 | 🟡（归存：P9 后另行排期——本仓为 `/title`（`--auto` 已可用，H-14 ✅）；`/rename` 别名未注册） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                       |
| G-66 | `/history`                                      | 历史浏览                                                                     | 必刻 | 🟡（归存：P9 后另行排期——有 `/timeline`（只读轨迹，仅 fullscreen）与 `/undo`；交互式历史浏览浮层未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）             |
| G-67 | `/home`（`/welcome`） · `/quit`（`/exit`）      | 首屏与退出                                                                   | 必刻 | 🟡（归存：P9 后另行排期——`/exit`（别名 `/quit`）已注册；`/home` 首屏命令未注册） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                   |

### 模型与模式

| ID   | 命令                            | 要点                                                   | 等级 | 状态                                                                                                                                                                                        |
| ---- | ------------------------------- | ------------------------------------------------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G-68 | `/model`（`/m`）                | 模型选择器，可带 effort 参数                           | 必刻 | 🟡（归存：P9 后另行排期——本仓无运行期模型选择器（模型由 config/provider 装配期确定，与 G-32 同因）） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                     |
| G-69 | `/effort low/medium/high/xhigh` | 推理强度（按模型能力）                                 | 必刻 | 🟡（归存：P9 后另行排期——有 `/reasoning on/off`（展示开关）；low/medium/high/xhigh 推理强度档位未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                    |
| G-70 | `/auto` · `/always-approve`     | **真开关**（非一次性命令），与 `Shift+Tab` 循环同源    | 必刻 | ✅                                                                                                                                                                                          |
| G-71 | `/plan [描述]` · `/view-plan`   | 计划模式与计划查看（`/show-plan` `/plan-view`）        | 必刻 | 🟡（归存：P9 后另行排期——审批模式含 plan 态（`/mode plan`、Shift+Tab 循环）；`/plan <描述>` 与 `/view-plan` 计划面未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评） |
| G-72 | `/multiline`（`/ml`）           | 多行输入开关                                           | 必刻 | 🟡（归存：P9 后另行排期——composer 有 Shift+Enter 换行；`/multiline` 开关未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                           |
| G-73 | `/vim-mode` · `/compact-mode`   | 输入模式与紧凑渲染                                     | 必刻 | 🟡（归存：P9 后另行排期——vim 模式未落（G-07）；compact-mode 紧凑渲染未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                               |
| G-74 | `/edit-prompt`                  | 外部编辑器写 prompt，顺序 `$VISUAL` → `$EDITOR` → `vi` | 必刻 | 🟡（归存：P9 后另行排期——无外部编辑器写 prompt（`$VISUAL`→`$EDITOR`→`vi`）面） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                           |
| G-75 | `/minimal` · `/fullscreen`      | 渲染模式切换（见 G-02）                                | 必刻 | ✅                                                                                                                                                                                          |
| G-76 | `/timestamps`                   | 时间戳显示开关                                         | 必刻 | 🟡（归存：P9 后另行排期——无时间戳显示开关） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                                                              |

### 记忆 · 扩展 · 调度 · 其他

| ID   | 命令                                                                                                    | 要点                                                                                                                                                                                                                                                                                                                                                                                                                      | 等级   | 状态                                                                                                                                                                                  |
| ---- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G-77 | `/memory`（`/mem` on/off） · `/remember` · `/flush` · `/dream`                                          | 记忆子系；需 `GROK_MEMORY=1` 或 `[memory] enabled`                                                                                                                                                                                                                                                                                                                                                                        | 补齐   | 🟡（归存：P9 后另行排期——`/memory` 只读查看已落（core catalog）；`/remember` `/flush` `/dream` 未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）              |
| G-78 | `/hooks` `/plugins` `/marketplace` `/skills` `/workflows`                                               | **同一个 extensions 模态的 5 个 tab**（不是 5 个独立窗口）                                                                                                                                                                                                                                                                                                                                                                | 必刻   | 🟡（归存：P9 后另行排期——`/plugins` `/skills` `/mcps` 只读命令已落；无统一 extensions 模态与 5 tab） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）               |
| G-79 | `/loop [间隔] <prompt>`                                                                                 | 定时自循环；间隔 `Ns`（≥60）/`Nm`/`Nh`/`Nd`，7 天过期；每次触发在**独立后台子代理**中运行（看不到会话上下文，prompt 必须自含，仅结果返回；2026-09-13 修正：原「每次触发创建新回合」已失真）                                                                                                                                                                                                                               | 补齐   | 🟡（归存：P9 后另行排期——cron（H-46）为等价能力（60s tick + 自然语言）；`/loop` 命令与独立后台子代理触发语义未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评） |
| G-80 | `/goal <目标> [--budget tokens]` + `status/pause/resume/clear`                                          | 长期目标与预算                                                                                                                                                                                                                                                                                                                                                                                                            | 补齐   | 🟡（归存：P9 后另行排期——无目标/预算面） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                                                           |
| G-81 | `/workflow <name>` + `runs/pause/resume/stop/save`                                                      | `.grok/workflows/*.rhai` 与 `~/.grok/workflows/*.rhai`；`agent_budget` 默认 128、范围 1–1024、并发上限 32；详情页 `p`/`r`/`x`/`s`                                                                                                                                                                                                                                                                                         | 补齐   | 🟡（归存：P9 后另行排期——无 workflow 运行时（`.rhai`）；core `flows/` 是特权/审批 flow，非该语义） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                 |
| G-82 | `/deep-research <query>`                                                                                | 深度研究流                                                                                                                                                                                                                                                                                                                                                                                                                | 补齐   | 🟡（归存：P9 后另行排期——无内置深度研究流） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                                                        |
...
| G-86 | `/docs`（`/howto` `/guides`） · `/tutorial`（`/tour` `/onboarding`） · `/release-notes`（`/changelog`） | 内置文档与引导                                                                                                                                                                                                                                                                                                                                                                                                            | 参考   | 🟡（归存：P9 后另行排期——`/doctor`（G-85 ✅）已有；`/docs` `/tutorial` `/release-notes` 未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                     |
| G-87 | `/settings`（`/config` `/preferences` `/prefs`）                                                        | 设置面板（与 `F2` 同源）                                                                                                                                                                                                                                                                                                                                                                                                  | 必刻   | 🟡（归存：P9 后另行排期——无设置面板（与 G-40 同因）；`/theme` `/mode` 为单点命令） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                 |
| G-88 | `/mcps` · `/config-agents`（`/agents`） · `/personas`                                                   | MCP 与子代理/人格配置                                                                                                                                                                                                                                                                                                                                                                                                     | 补齐   | 🟡（归存：P9 后另行排期——`/mcps` 只读已落；子代理/人格配置命令未落） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                                               |
| G-89 | `/btw` · `/feedback`                                                                                    | `/feedback` 重写（2026-09-13 修正）：裸命令在 full TUI 打开**独立反馈模态**（`Write`/`Drafts` 双 tab、类型/任务类别/失败模式枚举 picker、图片粘贴、草稿本地持久化、trace 同意卡、可被更高优先级阻塞卡片让位且归位恢复）；`/feedback <text>` 变为模型回合（先存本地草稿，让模型调 `send_feedback` 分类，禁止自称已发送）；minimal 无模态渲染器，只接受 `/feedback <text>` 并对裸命令给出可见拒绝；voice 占用输入时同样拒绝 | 参考   | 🟡（归存：P9 后另行排期——`/btw` 与独立反馈模态未落；minimal 下的可见拒绝行为随模态一并延后） （2026-09-17 本轮冻结：视觉复刻总纲 V0–V6 期间不排期，收口后重评）                       |
