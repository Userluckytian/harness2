...
| ID   | 能力           | 要点                                                             | 等级   | 状态 |
| ---- | -------------- | ---------------------------------------------------------------- | ------ | ---- |
| H-20 | 记忆文件       | `MEMORY.md`（事实）+ `USER.md`（用户模型），`/sethome` 定位      | 补齐   | ⬜   |
...
| session | `/model` `/sessions` `/compress` `/usage` `/insights [--days N]` `/skills` `/<skill-name>` `/stop` `/personality` `/sethome`                                                                                                                                                 | 补齐 |
| ops     | `/platforms` `/reset`                                                                                                                                                                                                                                                        | 待定 |
| debug   | `/heapdump` `/mem`                                                                                                                                                                                                                                                           | 参考 |
| 不采纳  | `/billing` `/credits` `/fortune`                                                                                                                                                                                                                                             | ➖   |

**H-70**：一个关键设计——斜杠命令分“本地处理”与“下沉内核”两层，壳只拦真正需要 UI 的（如 `/density` `/redraw`），其余统一交给内核。**harness2 必须照这个分层做，否则 CLI/桌面/web 三份命令表会分叉。**（必刻）
...
| 壳/内核边界 | 斜杠命令写在 CLI 包里（`command-registry.ts`），**桌面用不上**     | H-70（必须下沉） |
