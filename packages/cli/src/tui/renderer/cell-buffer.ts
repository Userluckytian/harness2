// cell-buffer.ts — 字符网格 buffer（方案 B 核心，P2 T2-1）。
//
// 每个单元格存 char + 显示宽度 + 前景色；宽字符占 2 列，续列格子宽度记 0、char 为空串。
//
...
/**
 * 字符网格 buffer：chars/widths/fg 三个平行数组按行优先存储。
 * 空白单元格：char=' '，width=0，fg=0。
 */
