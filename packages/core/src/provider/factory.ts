...
import { ConfigError, defaultConfigPaths, type AuthFile, type HarnessConfig, type ProviderConfig } from '../config/index.js';
...
export type KeySource =
  | { kind: 'auth.json'; key: string }
  | { kind: 'env'; envKey: string; key: string }
  | { kind: 'missing' };
...
export function resolveApiKey(
  ...
  if (provider.envKey) {
    ...
    if (typeof fromEnv === 'string' && fromEnv.length > 0) return { kind: 'env', envKey: provider.envKey, key: fromEnv };
  }
  return { kind: 'missing' };
}
...
export function createProvider(config: HarnessConfig, role: string, options: CreateProviderOptions = {}): ChatProvider {
  ...
  if (providerCfg.models && !(roleCfg.model in providerCfg.models)) {
    throw new ConfigError(
      `roles.${role}.model "${roleCfg.model}" 未在 providers.${roleCfg.channel}.models 中声明`,
    );
  }
