...
function attachWheelBridge(stdin, stdout, onWheel) {
  ...
  const flushAsLiteral = () => {
    ...
    const text = buf;
    buf = '';
    if (stdin.listenerCount('readable') > 1) { bypass = true; stdin.unshift(text); }
  };
  const handler = () => {
    ...
    if (forward.length > 0 && stdin.listenerCount('readable') > 1) { bypass = true; stdin.unshift(forward); }
    if (buf.length > 0) timer = setTimeout(flushAsLiteral, 40);
  };
  stdin.on('readable', handler);
  stdout.write('\x1b[?1000h\x1b[?1006h');
  return () => { stdin.removeListener('readable', handler); if (timer) clearTimeout(timer); stdout.write('\x1b[?1006l\x1b[?1000l'); };
}
...
function App({ wheel, log }) {
  ...
  useInput((input, key) => {
    ...
    if (key.return) { setAppended((a) => [...a, `> ${draft || '(empty)'}`]); setDraft(''); setFollow(true); return; }
    if (key.backspace || key.delete) { setDraft((d) => d.slice(0, -1)); return; }
    if (input === 'q' && draft === '') { process.exit(0); }
    if (!key.ctrl && input && !input.startsWith('\x1b')) { log.push(`echo:${JSON.stringify(input)}`); setDraft((d) => d + input); }
  });
  ...
  return h(Box, { flexDirection: 'column', width: columns },
    h(Box, { flexDirection: 'column', height: viewportH }, ...shown.map((line, i) => h(Text, { key: st + i, wrap: 'truncate' }, line))),
...
await sleep(150);
stdin.emitInput('k'); await sleep(120);                    // 上滚 1 行（vim k）
stdin.emitInput('\x1b[B'); await sleep(120);               // 下滚 1 行
stdin.emitInput('\x1b[<64;10;5M'); await sleep(120);       // 滚轮上 3 行（桥剥出）
stdin.emitInput('h'); await sleep(120);                    // 回显 'h'
stdin.emitInput('a'); await sleep(120);                    // 回显 'a'
stdin.emitInput('\r'); await sleep(120);                   // 发送 → appended=1, follow=true
stdin.emitInput('\x1b[<64;10;5M'); await sleep(120);       // 滚轮上 3 行
stdin.emitInput('\x1b[<65;10;5M'); await sleep(120);       // 滚轮下 3 行（9966→9969，仍未到底）
stdin.emitInput('\x1b[<65;10;5M'); await sleep(120);       // 9969→9972
stdin.emitInput('\x1b[<65;10;5M'); await sleep(120);       // 9972→9973（到底）→ FOLLOW
inst.unmount();
dispose();
