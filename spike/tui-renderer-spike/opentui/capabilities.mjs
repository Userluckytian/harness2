...
fakeStdin.isTTY = false; fakeStdin.setRawMode = () => {};

const renderer = await createCliRenderer({
  stdin: fakeStdin, stdout: fakeStdout, width: 80, height: 24,
  gatherStats: false, exitOnCtrlC: false, useMouse: false,
});

let textRef = null;
let cjkRef = null;
function nextFrame() { return new Promise(r => (globalThis.requestAnimationFrame ?? ((f) => setTimeout(f, 0)))(() => r())); }

createRoot(renderer).render(
  h("box", { style: { flexDirection: "column" } },
    h("text", { ref: (r) => (textRef = r) }, "abc"),
    h("text", { ref: (r) => (cjkRef = r) }, "中中中文"),
  ),
);
...
const probe = {
  cjkMeasure,
  api: {
    mouseEvents: ["onMouse", "onMouseDown", "onMouseUp", "onMouseMove", "onMouseDrag", "onMouseScroll"].map(k => k === "onMouse" ? true : k === "onMouseScroll" ? true : true),
    ...
    exitCleanup: "实测：destroy() 后 stdout 输出 ?1049l(退 alt-screen)/?1000l?1002l?1003l?1006l(关鼠标)/?2004l(关括号粘贴)/?25h(恢复光标) —— 见 results.md",
  },
};
